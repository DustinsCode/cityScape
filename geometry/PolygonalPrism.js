...
class PolygonalPrism extends Object3D {
    ...
    constructor (gl, props) { // topRadius, bottomRadius, height, radialDiv, verticalDiv, props.topColor, props.bottomColor) {
        ...
        if (typeof props.topRadius === "undefined")
            alert ("PolygonalPrism: missing value for topRadius");
        if (typeof props.bottomRadius === "undefined")
            alert ("PolygonalPrism: missing value for bottomRadius");
        if (typeof props.height === "undefined")
            alert ("PolygonalPrism: missing value for height");
        if (typeof props.numSides === "undefined")
            alert ("PolygonalPrism: missing value for numSides");
        let vertices = [], colors = [];
        const verticalDiv = 1;
